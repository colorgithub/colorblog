...
export function PostListClient({ posts }: { posts: Post[] }) {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    try {
      ...
      toast.success('文章已删除')
      router.refresh()
    } catch { toast.error('删除失败') } finally { setDeleting(null) }
  }
    ...
    <div className="rounded-2xl border bg-[hsl(var(--card))] overflow-hidden">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="divide-y">
          {filtered.map((post) => (
            <div key={post.id} className="flex items-center justify-between p-4 hover:bg-[hsl(var(--muted))/50] transition-colors">
              <div className="flex items-center gap-3 min-w-0">
                <div className={cn('w-2 h-2 rounded-full shrink-0', post.published ? 'bg-emerald-500' : 'bg-amber-500')}
                  title={post.published ? '已发布' : '草稿'} />
                <div className="min-w-0">
                  <p className="font-medium text-sm truncate">{post.title}</p>
                  <p className="text-xs text-[hsl(var(--muted-foreground))] mt-0.5">
                    {formatDate(post.createdAt)}{post.tags && ` · ${post.tags.split(',').map(t => t.trim()).join(', ')}`}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <Link href={`/posts/${post.slug}`} target="_blank"
                  className="p-2 rounded-lg hover:bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] transition-all" title="查看">
                  <ExternalLink size={16} />
                </Link>
                <Link href={`/admin/posts/${post.id}/edit`}
                  className="p-2 rounded-lg hover:bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--foreground))] transition-all" title="编辑">
                  <Edit size={16} />
                </Link>
                <button onClick={() => handleDelete(post.id, post.title)} disabled={deleting === post.id}
                  className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/50 text-[hsl(var(--muted-foreground))] hover:text-red-600 transition-all disabled:opacity-50" title="删除">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
