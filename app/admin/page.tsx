      ...
      <div className="pl-60 transition-all">
        <div className="max-w-5xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-3 gap-4 mb-10">
            {stats.map((s) => (
              <div key={s.label} className="rounded-2xl border bg-[hsl(var(--card))] p-5">
          ...
          <div className="rounded-2xl border bg-[hsl(var(--card))]">
            <div className="flex items-center justify-between p-5 border-b">
              ...
              <Link href="/admin/posts" className="text-sm text-[hsl(var(--accent))] hover:underline">查看全部</Link>
            </div>
            <div className="divide-y">
              {recentPosts.length === 0 ? (
                <div className="p-5 text-center text-sm text-[hsl(var(--muted-foreground))]">还没有文章</div>
              ) : (
                  ...
                  <div key={post.id} className="flex items-center justify-between p-5 hover:bg-[hsl(var(--muted))/50] transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[hsl(var(--muted-foreground))] mt-0.5">{formatDate(post.createdAt)}</p>
                      </div>
                    </div>
                    <Link href={`/admin/posts/${post.id}/edit`} className="text-xs text-[hsl(var(--accent))] hover:underline">编辑</Link>
                  </div>
                ))
              )}
            </div>
